...
/** Форма «Создать матч» вынесена из StepVkListStartPanel — меньше строк в родителе для ESLint max-lines. */
defineProps<{
...
const vkEventDate = defineModel<string>('vkEventDate', { required: true })
const vkEventTime = defineModel<string>('vkEventTime', { required: true })

const emit = defineEmits<{
  'select-preset': [preset: VkListPreset]
  'tr-slots-input': [value: string]
  'submit-create-match': []
}>()
...
function onTrTeamSlotsInput(value: string) {
  emit('tr-slots-input', value)
}

function submitCreateMatch() {
  emit('submit-create-match')
}
</script>
